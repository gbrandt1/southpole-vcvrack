...
void Gnome::process(const ProcessArgs &args) {
  ...
  float interp;
  if (wave < 1.0)
    interp = crossfade(.5 - lfo.saw(), lfo.tri(), wave);
  else if (wave < 2.0)
    interp = crossfade(lfo.tri(), lfo.saw(), wave - 1.0);
  else if (wave < 3.0)
    interp = crossfade(lfo.saw(), lfo.sqr(), wave - 2.0);
  else
    interp = crossfade(lfo.sqr(), lfo.sh(), wave - 3.0);
  ...
  float osc = 0.;
  if (oscwave < 1.0)
    osc = crossfade(tri, saw, oscwave);
  else
    osc = crossfade(saw, sqr, oscwave - 1.0);
  ...
  if (subwave < 1.0)
    sub = crossfade(sub1, sub2, subwave);
  else
    sub = crossfade(sub2, noise, subwave - 1.0);
  ...
  float freq = clamp(
      inputs[VCFFREQ_INPUT].getNormalVoltage(0.) + params[VCFPITCH_PARAM].getValue() + params[VCFENV_PARAM].getValue() * 11. * env + params[VCFLFO_PARAM].getValue() * 11. * interp, -4.0f, 6.0f);
  ...
  ntfilter.setCutoffFreq(cutoff);

  lpfilter.setResonance(reso * .995);
  bpfilter.setResonance(reso * .995);
  hpfilter.setResonance(reso * .995);
  ntfilter.setResonance(reso * .995);

  float lpout = lpfilter.processAudioSample(vco_out, 1);
  float bpout = bpfilter.processAudioSample(vco_out, 1);
  ...
  float fout;
  if (ftyp < 1.0)
    fout = crossfade(lpout, bpout, ftyp);
  else if (ftyp < 2.0)
    fout = crossfade(bpout, hpout, ftyp - 1.0);
  else
    fout = crossfade(hpout, ntout, ftyp - 2.0);
