...
struct EtagereWidget : ModuleWidget { 
    ...
    EtagereWidget(Etagere *module)  {
...

        {
            ...
            noirPanel->setBackground(APP->window->loadSvg(assetPlugin(pluginInstance, "res/Etagere.svg")));
            noirPanel->box.size = box.size;
            addChild(noirPanel);	
        }
        {
            blancPanel = new SVGPanel();
            blancPanel->setBackground(APP->window->loadSvg(assetPlugin(pluginInstance, "res/Etagere_blanc.svg")));
