...
struct sp_Port : SVGPort {
	sp_Port() {
		setSVG(SVG::load(assetPlugin(pluginInstance, "res/sp-Port20.svg")));
	}
};
...
struct sp_Encoder : SVGKnob {
	sp_Encoder() {
        minAngle = -0.83 * M_PI;
		maxAngle = 0.83 * M_PI;
		setSVG(SVG::load(assetPlugin(pluginInstance, "res/sp-encoder.svg")));
		//sw->svg = SVG::load(assetPlugin(pluginInstance, "res/sp-encoder.svg"));
		//sw->wrap();
...
struct sp_BlackKnob : SVGKnob {
	sp_BlackKnob() {
        minAngle = -0.83 * M_PI;
		maxAngle = 0.83 * M_PI;
		setSVG(SVG::load(assetPlugin(pluginInstance, "res/sp-knobBlack.svg")));
		//sw->svg = SVG::load(assetPlugin(pluginInstance, "res/sp-knobBlack.svg"));
		//sw->wrap();
		//box.size = Vec(32,32);
	}
};
...
struct sp_Trimpot : SVGKnob {
	sp_Trimpot() {
        minAngle = -0.83 * M_PI;
		maxAngle = 0.83 * M_PI;
		setSVG(SVG::load(assetPlugin(pluginInstance, "res/sp-trimpotBlack.svg")));
		//sw->svg = SVG::load(assetPlugin(pluginInstance, "res/sp-knobBlack.svg"));
		//sw->wrap();
		//box.size = Vec(18,18);
	}
